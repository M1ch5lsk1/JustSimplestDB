...
class Instance:
    def __init__(self, file_to_read: owntypes.FileName, do_save_txt_as_db: bool = False) -> None:
        ...
        self.__shell = self.main_file + "_shell" + ".py"
        self.__is_shell_exists = False
    ...
    def __create_dbms_shell(self):
        if not self.__is_shell_exists:
            with open(self.__shell, "w", encoding="UTF-8") as file:
                file.write("import JustSimplestDB\n")
                file.write(f"db = JustSimplestDB.Instance({self.main_file})\n")
                file.write("user_query = input(\"Your query: \")")
                file.write()
